...
bs = 64
dataset = QADataset('/home/xuehongyang/mkdata/train.pkl')
valset = QADataset('/home/xuehongyang/mkdata/val.pkl')
testset = QADataset('/home/xuehongyang/mkdata/test.pkl')
...
treemn = TreeMN(4096, 300, 1024, 500)
val_batches = math.ceil(7684 // bs)
train_batches = math.ceil(24415 // bs)
test_batches = math.ceil(16258 // bs)
if params.use_cuda:
    treemn.cuda()
n_epoch = 300
optimizer = th.optim.Adam(treemn.parameters(), lr=0.0001)
...
for epoch in range(n_epoch):
    ...
    print('current best epoch: %d, test accuracy = %f' % (best_epoch,
                                                          best_test))
    try:
        itchat.send('The TreeMN-1024 model: epoch = %d,'
